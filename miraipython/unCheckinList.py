...
'''  ##########！！！配置信息！！！(务必修改)########## '''
USERNAME = "0100485"  # 本地打卡更改or后面就行
PASSWORD = "20120809Lyn."  # 本地打卡更改or后面就行
# 学校拼音简写命名变量，真的会谢
checkin_data = {
...
if "WXPUSHERTOPICIDS" in os.environ:
    wx_pushertopic_ids = os.environ["WXPUSHERTOPICIDS"].split(",")
...
def get_stu_check_data(session: requests.Session):
    url = "http://ehall.cqupt.edu.cn/publicapp/sys/cyxsjkdk/modules/jqdkcx/jqdklb.do"
    form_data = {
        "querySetting": [
            {"name": "RQ", "caption": "日期", "builder": "lessEqual", "linkOpt": "AND", "builderList": "cbl_Other","value": datetime.datetime.now().strftime("%Y-%m-%d")},
             {"name": "RQ", "caption": "日期", "linkOpt": "AND", "builderList": "cbl_String", "builder": "moreEqual","value": datetime.datetime.now().strftime("%Y-%m-%d")},
            {"name": "pageNumber", "value": 1, "linkOpt": "AND", "builder": "equal"}
        ],
        "pageNumber": "1",
        "pageSize": "620",
    }
    ...
    print(res_json)
    try:
        today_data = res_json["datas"]["jqdklb"]["rows"]
        print(today_data)
    except Exception as e:
...
# 判断今日是否已经打卡，若未打卡返回打卡相关信息，包括打卡所需：WID字段、学号字段、姓名字段
def get_today_info(session: requests.Session) -> object:
...
def mainCheck():
    ...
    while current_try_times < try_times:
        ...
        temp_session = _session  # 防止错误的session环境，影响下一次尝试
        result, today_data = get_stu_check_data(session=temp_session)
        print(today_data)
        if result == 0:
            postNotice({'msg_type': 'group', 'number': '706432386', 'msg': "今日同学已全部打卡！"})
            print(result)
            _session = temp_session
            break
        elif result == 1:
            msgList =[]
            for json in today_data:
                msgList.append(str(json.XM)+'：'+str(json.XH))
            postNotice({'msg_type': 'group', 'number': '706432386', 'msg': "今日未打卡学生名单："+str(msgList)})
